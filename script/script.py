...
import zipfile
import time
choices = ['bungalow', 'tl', 'tacoticklers', 'laundromat', 'money', 'spawn', 'instantgrowth', 'autoplant']
        ...
        # Check if the Data.json file exists
            ...
            # Open and read the Data.json file
                        with open(data_filepath, 'r') as file:
                            data = json.load(file)
                        if data['SoilID'] == '' or 'SoilLevel' == 0.0 :
                            data['SoilID'] = 'extralonglifesoil'
                            data['SoilLevel'] = 20.0
                            data['RemainingSoilUses'] = 30
                            print(f'Added soil to {data_filepath}')
            # Modify the growthprogress value to 100.0
                        if 'growthprogress' in data and 'SeedID' in data and 'YieldLevel' in data and 'QualityLevel' in data:
                            data['SeedID'] = schoice
                            data['growthprogress'] = 1
                            data['YieldLevel'] = 0.6700000166893005
                            data['QualityLevel'] = 0.44999998807907107
                        else:
                            print(f"No 'growthprogress' or 'SeedID' or 'YieldLevel' or 'QualityLevel' key found in {data_filepath}. Skipping...")
                            continue
